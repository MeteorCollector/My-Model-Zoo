...
from model import MyYOLO, yolo_loss
...
class CFG:
    ...
    root0712 = [r'../data/VOCdevkit/VOC2007', r'../data/VOCdevkit/VOC2012']
...
def train():
    ...
    yolo_net = MyYOLO(S=CFG.S, out_channel=CFG.B * 5 + CFG.num_classes)
    yolo_net.to(device)
    ...
    for epoch in range(CFG.start_epoch, CFG.epoch):
        ...
        if best_train_loss > loss_score.avg:
            print('Save yolo_net to {}'.format(os.path.join(CFG.model_root, 'yolo{:.4f}.pth'.format(loss_score.avg))))
            torch.save(yolo_net.state_dict(), os.path.join(CFG.model_root, 'yolo{:.4f}.pth'.format(loss_score.avg)))
